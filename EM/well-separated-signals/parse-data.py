...
fout = open("Data/output-EM_well-separated-signals.dat", "w")
    ...
    for r in range(num_run):
        ...
        for filename in filelist:
            ...
            if (file_count == 0):
                ...
                sol = np.array(lines[2*num_sol-2].split()).astype(np.double)
                prob = np.array(lines[2*num_sol-1].split()).astype(np.double)
            else:
